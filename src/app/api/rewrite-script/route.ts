...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await rewriteScript({
      ...
      tone:
        tone?.trim() ||
        "conversational, direct, short-form video ad",
    });

    return NextResponse.json(result);
  } catch (error) {
    if (isRewriteScriptError(error)) {
      return NextResponse.json(error, { status: 500 });
    }

    const message =
      error instanceof Error ? error.message : "Script rewrite failed.";
    return NextResponse.json({ error: message }, { status: 500 });
  }
}

function isRewriteScriptError(error: unknown): error is RewriteScriptError {
  return (
    typeof error === "object" &&
    error !== null &&
    "phase" in error &&
    (error as RewriteScriptError).phase === "script"
  );
}
